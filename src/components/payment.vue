...
export default {
  ...
  methods: {
    operation (value) {
      this.$router.push({path: '/pay', query: {oid: value.oid, sumPirce: value.productPrice * value.productNum}})
    },
    ...
    getOrderList () {
      this.$axios.get(`${this.restUrl}/order/userOrderStatus?status=1`).then((res) => {
        console.log(res)
        this.orderList = []
        for (var i = 0; i < res.data.data.length; i++) {
          var obj = {
